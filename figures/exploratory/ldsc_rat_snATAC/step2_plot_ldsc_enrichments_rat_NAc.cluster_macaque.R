...
col_celltype = c(RColorBrewer::brewer.pal(5, 'Paired'),
...
enrichments = input %>% 
  mutate(
    ...
    cell_group = case_when(
      grepl('Shell', celltype) ~ 'Canonical MSN', 
      grepl('^D', celltype) ~ 'Non-canonical MSN', 
...
plot_fn = here(FIGDIR, 'plots','Rat_Transgen_NAc_ldsc.cluster_macaque.bonf.ppt.pdf')
...
plot_fn = here(FIGDIR, 'plots','Rat_Transgen_NAc_ldsc.cluster_macaque.FDR.ppt.pdf')
pdf(width = width_ppt, height = height_ppt, file = plot_fn)
for(plotme in list(c('Psych',"SUD", 'Neuro','Sleep'), 
                   c('SU','Degen','Metabolism','Other'))){
  pp = ggplot(data = enrichments %>% filter(group %in% plotme),
